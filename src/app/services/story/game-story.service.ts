...
export class GameStoryService {
  ...
  checkLevelTrigger(level: LevelStory): void {
    switch (level.effect) {
      case 'extraArrow':
        this.gameStore.updateHunter({ arrows: this.gameStore.arrows() + 1 });
        break;
      case 'extraLife':
        this.gameStore.updateHunter({ lives: this.gameStore.lives() + 1 });
        break;
      case 'doubleGold':
        this.gameStore.updateHunter({ gold: this.gameStore.gold() + 200 });
        break;
      default:
        break;
    }
