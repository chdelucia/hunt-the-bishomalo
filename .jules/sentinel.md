...
## 2025-05-23 - [Prototype Pollution in MiniBusService and LocalStorageService]
**Vulnerability:** The `MiniBusService` event store was vulnerable if pre-initialized as a plain object. `LocalstorageService` did not sanitize JSON-parsed data, potentially allowing prototype pollution from tampered local storage data.
**Learning:** Defense-in-depth requires ensuring that even "protected" objects are truly prototype-less and that all external/persisted data is sanitized before use, even if it's "just" JSON.
**Prevention:** Always ensure global stores are prototype-less using `Object.create(null)` and `Object.defineProperty`. Recursively sanitize objects from untrusted sources (like `localStorage`) to strip `__proto__`, `constructor`, and `prototype` keys.
