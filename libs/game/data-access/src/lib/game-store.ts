...
import { LocalstorageService, GameSoundService } from '@hunt-the-bishomalo/core';
import { GameSettings, Chars, GameState, GameItem, Hunter, GameSound } from '@hunt-the-bishomalo/data';
...
export const GameStore = signalStore(
  ...
  withMethods((store, localStorage = inject(LocalstorageService), sound = inject(GameSoundService)) => {
    ...
    return {
      ...
      $_handleGameOverSound() {
        if (!store.lives()) {
          sound.stop();
          sound.playSound(GameSound.GAMEOVER, false);
        }
      }
    };
  }),
  withHooks({
    onInit(store) {
      ...
      effect(() => store.$_handleGameOverSound());
    },
  }),
);
